...
import { IncomeStackedBarChart } from "./IncomeStackedBarChart";
...
export function IncomeCategoryChart({ analytics }: Props) {
  ...
  useEffect(() => {
    ...
    if (!stillValid) {
      ...
      console.info("[IncomeCategoryChart] reset to all — account no longer in data");
    }
  }, [accounts, accountId]);
  ...
  if (!hasData) {
    return (
      <div className="card" style={{ marginBottom: 16 }}>
        <h3 style={{ marginTop: 0 }}>Income by category (5 months)</h3>
        <p className="note">No income data in this period.</p>
      </div>
    );
  }
  ...
  return (
    <div className="card" style={{ marginBottom: 16 }}>
      ...
      >
        <h3 style={{ margin: 0 }}>Income by category (5 months)</h3>
        <label style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              console.info("[IncomeCategoryChart] selected", e.target.value);
            }}
          >
