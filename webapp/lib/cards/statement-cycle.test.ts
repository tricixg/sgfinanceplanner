...
describe("statement-cycle", () => {
  it("May 21 statement covers Apr 21 – May 20", () => {
    const { cycleStart, cycleEnd } = cycleBoundsFromClose("2026-05-21", 21);
    expect(cycleStart).toBe("2026-04-21");
    expect(cycleEnd).toBe("2026-05-20");
  });
  ...
  it("assigns spend on day after cycle end to next statement", () => {
    expect(statementCloseForSpend("2026-05-21", 21)).toBe("2026-06-21");
  });
  ...
  it("open cycle after statement day in May", () => {
    ...
    expect(cycleStart).toBe("2026-05-21");
    expect(cycleEnd).toBe("2026-06-20");
